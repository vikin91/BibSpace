...
try {
    ...
    my %backendConfig = (
        backends => [
            { prio => 1, type => 'BibSpace::Model::DAO::ArrayDAOFactory', handle => "NOHANDLE" },
            { prio => 2, type => 'BibSpace::Model::DAO::RedisDAOFactory', handle => $redisHandle },
            { prio => 2, type => 'BibSpace::Model::DAO::MySQLDAOFactory', handle => $dbHandle },
        ]
    );
