...
subtest 'Fregenerate_html_for_array' => sub {
  ...
  is( Fregenerate_html_for_array($app, 1, $app->bibtexConverter, \@entries), 1, "regen 1 entry ok");

  @entries = @all_entries[1..2];
  is( Fregenerate_html_for_array($app, 1, $app->bibtexConverter, \@entries), 2, "regen 2 entries ok");

  @entries = @all_entries[3..5];
  is( Fregenerate_html_for_array($app, 1, $app->bibtexConverter, \@entries), 3, "regen 3 entries ok");

  @entries = ();
  is( Fregenerate_html_for_array($app, 1, $app->bibtexConverter, \@entries), 0, "regen 0 entries ok");
