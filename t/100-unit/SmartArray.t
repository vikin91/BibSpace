...
my $backend = SmartArray->new( logger => SimpleLogger->new() );
...
is($found->str, "updated", "found new updated object");

is( $backend->delete($thing3), 1, "delete ok" );

my $found = $backend->find($type, sub{$_->id == 3});
ok(!$found, "not found");
is( scalar($backend->filter($type, sub{$_->id < 100}) ), 2, "filter after delete ok" );
