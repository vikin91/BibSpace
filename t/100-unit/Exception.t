...
note "============ Testing ".scalar(@all_exceptions)." entries ============";
...
foreach my $exception (@all_exceptions) {
  ...
  lives_ok( sub { $exception->equals_obj($exception) }, 'equals_obj expecting to live' );
  lives_ok( sub { $exception->equals_id($exception) }, 'equals_id expecting to live' );
