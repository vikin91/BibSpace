...
foreach my $type (@all_types){
  ...
  if( $type->bibtexTypes_count == 1){
    ...
    if( $type->get_first_bibtex_type eq $type->our_type ){
      ok( !$type->can_be_deleted, "allowed deletion of custom 1:1 mapping \n\t".$type->toString."\t - should not allow to delete");
    }
    else{
      ok( $type->can_be_deleted, "disalowed deletion of custom 1:1 mapping \n\t".$type->toString."\t - should allow to delete");
    }
    ...
    ok( !$type->can_be_deleted, "allowed deletion of 1:N mapping \n\t".$type->toString."\t - should not allow to delete");
