...
my $mock_obj_users       =  $self->app->entityFactory->new_User( login=>'test', email=>'test', pass=>'test', pass2=>'test' );
...
foreach my $obj (@objects){
  ...
  foreach my $action (@actions1){
    note "================= Testing action $action =================";
    my $code = '$t_anyone->app->repo->'."$obj"."_"."$action(sub {defined ".'$_'."} );";
    my $res = eval "$code";
    isnt( $res, '', "'$code'. Output: '$res'. Error captured: '$@'." );
  } 
  ...
  foreach my $action (@actions2){
    ...
    $code .= '$t_anyone->app->repo->'."$obj"."_"."$action( ".'$element'." );";
