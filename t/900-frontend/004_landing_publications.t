...
my @tags = $t_logged_in->app->repo->tags_filter(sub{ scalar($_->get_entries)>0 });
my $some_tag = $tags[0];

my @tags_permalink = $t_logged_in->app->repo->tags_filter(sub{ defined $_->permalink and length($_->permalink)>1 });
my $some_permalink = $tags_permalink[0];

my @teams = $t_logged_in->app->repo->teams_all;
my $some_team = $teams[0];

my @authors = $t_logged_in->app->repo->authors_filter(sub{ scalar($_->get_entries) > 0});
...
my @pages = (
	...
	$self->url_for('lp')->query(entry_type => 'paper', author => $some_author->uid),
	$self->url_for('lp')->query(entry_type => 'paper', author => $some_author->id),
	...
	$self->url_for('lp')->query(entry_type => 'paper', permalink => $some_permalink->name),
	$self->url_for('lp')->query(entry_type => 'paper', permalink => $some_permalink->id),
	$self->url_for('lp')->query(entry_type => 'paper', permalink => 'not-existing'),
	$self->url_for('lp')->query(entry_type => 'paper', bibtex_type => 'inproceedings'),
	$self->url_for('lp')->query(entry_type => 'talk'),

	$self->url_for('lyp'),
	$self->url_for('lyp')->query(entry_type => 'paper'),
	$self->url_for('lyp')->query(entry_type => 'paper', author => $some_author->uid),
	$self->url_for('lyp')->query(entry_type => 'paper', author => $some_author->id),
	...
	$self->url_for('lyp')->query(entry_type => 'paper', permalink => $some_permalink->name),
	$self->url_for('lyp')->query(entry_type => 'paper', permalink => $some_permalink->id),
	$self->url_for('lyp')->query(entry_type => 'paper', permalink => 'not-existing'),
	$self->url_for('lyp')->query(entry_type => 'paper', bibtex_type => 'inproceedings'),
