...
{    # Closure
  ...
  my %Styles = (em => 'em', bf => 'b', it => 'i', sl => 'i', tt => 'tt');
  ...
  $HTML = {
    ...
    math => sub {
      ...
      $ASCIIMathML_parser = Text::ASCIIMathML->new() unless $ASCIIMathML_parser;
    ...
    style => sub {
      ...
      return defined $html_style ? "<$html_style>$text</$html_style>" : $text;
    },
  };
}
{    # Closure
  my %Styles = (em => '*', bf => '**', it => '*', sl => '*', tt => '``');
...
################ Internal routines
   ...
   # Inputs: $self, command name, file name, line num, ref to array of arguments
    sub _command_entry {
      ...
      $self->{bibtex}{format} = {
        field => [
          'crossref',
          map($self->_check_name($_), grep !/^\#line/, @{$args_ar->[0]})
        ],
        integer =>
          [map($self->_check_name($_), grep !/^\#line/, @{$args_ar->[1]})],
        string => [
          'sort.key$',
          map($self->_check_name($_), grep !/^\#line/, @{$args_ar->[2]})
        ]
      };
  ...
  # print "_read_from_raw: Processing raw Entry metatype $metatype key $key \n";
    ...
    sub _command_variables {
      ...
      $self->{symbols}{$cmd}{$_} = $init
        foreach map($self->_check_name($_), grep !/^\#line/, @{$args_ar->[0]});
    }
  ...
  sub _format_token : method {
    ...
    $_ = $token;
    return (!defined $_ || $_ eq "-0" ? 'missing'
      : /^-?\d+$/ ? "#$_"
      : ref($_)   ? "{"
        . join(' ', map $self->_format_token($_), grep(!/\#line/, @$_)) . "}"
      : $_);
  }
  ...
  sub _function_format_name {
    ...
    foreach (@format) {
      if (/^PROTECT/) {
        ...
        my $name = $name{$form};
        next unless @$name;
        $pre = ' ' if $pre eq '' && $output ne '' && $output !~ /[ ~]$/;
        my $need_tie = 0;
        my @out_names;
        foreach (@$name) {
        ...
        my $out_tie = !$long && ($must_tie || $may_tie && $need_tie) ? '~' : '';
        $output .= ($pre || '');
...
################# Routines to support output translation
  ...
  {    # Closure
    ...
    my %Accents = (
      ...
      H => {O => 0x150, o => 0x151, U => 0x170, u => 0x171,},
      u => {
        A => 0x102,
        a => 0x103,
        E => 0x114,
