...
sub copy{
    ...
    $self->logger->info("Copying all Exception from layer $fromLayer to layer $toLayer.","".__PACKAGE__."->copy");
    ...
    )->getExceptionDao($self->getIdProvider)->all();

    $self->logger->info(scalar(@resultRead)." Exception read from layer $fromLayer.","".__PACKAGE__."->copy");
    ...
    )->getExceptionDao($self->getIdProvider)->save( @resultRead );

    $self->logger->info(" $resultSave Exception saved to layer $toLayer.","".__PACKAGE__."->copy");
}
...
sub all {
    ...
    try{
        return $self->backendDaoFactory
            ...
            ->getExceptionDao($self->getIdProvider)
            ->all();
    }
    catch{
        print;
