...
package BibSpace::Model::Repository::Layered::AuthorsLayeredRepository;
use namespace::autoclean;
use Moose;
require BibSpace::Model::Repository::Interface::IAuthorsRepository;
with 'BibSpace::Model::Repository::Interface::IAuthorsRepository';
...
sub all {
    ...
    try{
        $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getAuthorDao()->all();
    }
    catch{
        print;
    };
...
sub save {
    ...
    foreach my $backendDAO ( $self->getBackendsArray() ){
        ...
        try{
            $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getAuthorDao()->save( @objects );
        }
        catch{
            print;
        };
...
sub update {
    ...
    foreach my $backendDAO ( $self->getBackendsArray() ){
        ...
        my $daoBackendHandle = $backendDAO->{'handle'};
        try{
            $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getAuthorDao()->update( @objects );
        }
        catch{
            print;
        };
...
sub delete {
    ...
    foreach my $backendDAO ( $self->getBackendsArray() ){
        ...
        my $daoBackendHandle = $backendDAO->{'handle'};
        try{
            $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getAuthorDao()->delete( @objects );
        }
        catch{
            print;
        };
...
sub exists {
    ...
    foreach my $backendDAO ( $self->getBackendsArray() ){
        ...
        my $daoBackendHandle = $backendDAO->{'handle'};
        try{
            $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getAuthorDao()->exists( @objects );
        }
        catch{
            print;
        };
...
sub filter {
    ...
    my $daoBackendHandle = $self->_getReadBackend()->{'handle'};
    try{
        $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getAuthorDao()->filter( $coderef );
    }
    catch{
        print;
    };
...
sub find {
    ...
    my $daoBackendHandle = $self->_getReadBackend()->{'handle'};
    try{
        $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getAuthorDao()->find( $coderef );
    }
    catch{
        print;
    };
...
sub count {
    ...
    my $daoBackendHandle = $self->_getReadBackend()->{'handle'};
    try{
        $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getAuthorDao()->count( $coderef );
    }
    catch{
        print;
    };
