...
package BibSpace::Model::Repository::Layered::LabellingsLayeredRepository;
use namespace::autoclean;
use Moose;
require BibSpace::Model::Repository::Interface::ILabellingsRepository;
with 'BibSpace::Model::Repository::Interface::ILabellingsRepository';
...
sub all {
    ...
    try{
        $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getLabelingDao()->all();
    }
    catch{
        print;
    };
...
sub save {
    ...
    foreach my $backendDAO ( $self->getBackendsArray() ){
        ...
        try{
            $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getLabelingDao()->save( @objects );
        }
        catch{
            print;
        };
...
sub update {
    ...
    foreach my $backendDAO ( $self->getBackendsArray() ){
        ...
        my $daoBackendHandle = $backendDAO->{'handle'};
        try{
            $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getLabelingDao()->update( @objects );
        }
        catch{
            print;
        };
...
sub delete {
    ...
    foreach my $backendDAO ( $self->getBackendsArray() ){
        ...
        my $daoBackendHandle = $backendDAO->{'handle'};
        try{
            $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getLabelingDao()->delete( @objects );
        }
        catch{
            print;
        };
...
sub exists {
    ...
    foreach my $backendDAO ( $self->getBackendsArray() ){
        ...
        my $daoBackendHandle = $backendDAO->{'handle'};
        try{
            $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getLabelingDao()->exists( @objects );
        }
        catch{
            print;
        };
...
sub filter {
    ...
    my $daoBackendHandle = $self->_getReadBackend()->{'handle'};
    try{
        $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getLabelingDao()->filter( $coderef );
    }
    catch{
        print;
    };
...
sub find {
    ...
    my $daoBackendHandle = $self->_getReadBackend()->{'handle'};
    try{
        $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getLabelingDao()->find( $coderef );
    }
    catch{
        print;
    };
...
sub count {
    ...
    my $daoBackendHandle = $self->_getReadBackend()->{'handle'};
    try{
        $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getLabelingDao()->count( $coderef );
    }
    catch{
        print;
    };
