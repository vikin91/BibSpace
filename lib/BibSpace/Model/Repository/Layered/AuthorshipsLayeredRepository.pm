...
package BibSpace::Model::Repository::Layered::AuthorshipsLayeredRepository;
use namespace::autoclean;
use Moose;
require BibSpace::Model::Repository::Interface::IAuthorshipsRepository;
with 'BibSpace::Model::Repository::Interface::IAuthorshipsRepository';
...
sub all {
    ...
    try{
        $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getAuthorshipDao()->all();
    }
    catch{
        print;
    };
...
sub filter {
    ...
    my $daoBackendHandle = $self->_getReadBackend()->{'handle'};
    try{
        $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getAuthorshipDao()->filter( $coderef );
    }
    catch{
        print;
    };
...
sub find {
    ...
    my $daoBackendHandle = $self->_getReadBackend()->{'handle'};
    try{
        $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getAuthorshipDao()->find( $coderef );
    }
    catch{
        print;
    };
...
sub count {
    ...
    my $daoBackendHandle = $self->_getReadBackend()->{'handle'};
    try{
        $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getAuthorshipDao()->count( $coderef );
    }
    catch{
        print;
    };
