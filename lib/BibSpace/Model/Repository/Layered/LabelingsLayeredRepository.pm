...
sub copy{
    ...
    $self->logger->info("Copying all Labeling from layer $fromLayer to layer $toLayer.","".__PACKAGE__."->copy");
    ...
    $self->logger->info(scalar(@resultRead)." Labeling read from layer $fromLayer.","".__PACKAGE__."->copy");
    ...
    $self->logger->info(" $resultSave Labeling saved to layer $toLayer.","".__PACKAGE__."->copy");
}
