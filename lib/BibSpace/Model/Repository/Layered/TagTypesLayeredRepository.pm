...
package BibSpace::Model::Repository::Layered::TagTypesLayeredRepository;
use namespace::autoclean;
use Moose;
require BibSpace::Model::Repository::Interface::ITagTypesRepository;
with 'BibSpace::Model::Repository::Interface::ITagTypesRepository';
...
sub all {
    ...
    try{
        $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getTagTypeDao()->all();
    }
    catch{
        print;
    };
...
sub save {
    ...
    foreach my $backendDAO ( $self->getBackendsArray() ){
        ...
        try{
            $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getTagTypeDao()->save( @objects );
        }
        catch{
            print;
        };
...
sub update {
    ...
    foreach my $backendDAO ( $self->getBackendsArray() ){
        ...
        my $daoBackendHandle = $backendDAO->{'handle'};
        try{
            $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getTagTypeDao()->update( @objects );
        }
        catch{
            print;
        };
...
sub delete {
    ...
    foreach my $backendDAO ( $self->getBackendsArray() ){
        ...
        my $daoBackendHandle = $backendDAO->{'handle'};
        try{
            $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getTagTypeDao()->delete( @objects );
        }
        catch{
            print;
        };
...
sub exists {
    ...
    foreach my $backendDAO ( $self->getBackendsArray() ){
        ...
        my $daoBackendHandle = $backendDAO->{'handle'};
        try{
            $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getTagTypeDao()->exists( @objects );
        }
        catch{
            print;
        };
...
sub filter {
    ...
    my $daoBackendHandle = $self->_getReadBackend()->{'handle'};
    try{
        $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getTagTypeDao()->filter( $coderef );
    }
    catch{
        print;
    };
...
sub find {
    ...
    my $daoBackendHandle = $self->_getReadBackend()->{'handle'};
    try{
        $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getTagTypeDao()->find( $coderef );
    }
    catch{
        print;
    };
...
sub count {
    ...
    my $daoBackendHandle = $self->_getReadBackend()->{'handle'};
    try{
        $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getTagTypeDao()->count( $coderef );
    }
    catch{
        print;
    };
