...
sub copy{
    ...
    $self->logger->info("Copying all Team from layer $fromLayer to layer $toLayer.","".__PACKAGE__."->copy");
    ...
    )->getTeamDao($self->getIdProvider)->all();

    $self->logger->info(scalar(@resultRead)." Team read from layer $fromLayer.","".__PACKAGE__."->copy");
    ...
    )->getTeamDao($self->getIdProvider)->save( @resultRead );

    $self->logger->info(" $resultSave Team saved to layer $toLayer.","".__PACKAGE__."->copy");
}
