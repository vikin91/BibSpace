...
package BibSpace::Model::Repository::Layered::TeamsLayeredRepository;
use namespace::autoclean;
use Moose;
require BibSpace::Model::Repository::Interface::ITeamsRepository;
with 'BibSpace::Model::Repository::Interface::ITeamsRepository';
...
sub all {
    ...
    try{
        $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getTeamDao()->all();
    }
    catch{
        print;
    };
...
sub save {
    ...
    foreach my $backendDAO ( $self->getBackendsArray() ){
        ...
        try{
            $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getTeamDao()->save( @objects );
        }
        catch{
            print;
        };
...
sub update {
    ...
    foreach my $backendDAO ( $self->getBackendsArray() ){
        ...
        my $daoBackendHandle = $backendDAO->{'handle'};
        try{
            $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getTeamDao()->update( @objects );
        }
        catch{
            print;
        };
...
sub delete {
    ...
    foreach my $backendDAO ( $self->getBackendsArray() ){
        ...
        my $daoBackendHandle = $backendDAO->{'handle'};
        try{
            $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getTeamDao()->delete( @objects );
        }
        catch{
            print;
        };
...
sub exists {
    ...
    foreach my $backendDAO ( $self->getBackendsArray() ){
        ...
        my $daoBackendHandle = $backendDAO->{'handle'};
        try{
            $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getTeamDao()->exists( @objects );
        }
        catch{
            print;
        };
...
sub filter {
    ...
    my $daoBackendHandle = $self->_getReadBackend()->{'handle'};
    try{
        $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getTeamDao()->filter( $coderef );
    }
    catch{
        print;
    };
...
sub find {
    ...
    my $daoBackendHandle = $self->_getReadBackend()->{'handle'};
    try{
        $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getTeamDao()->find( $coderef );
    }
    catch{
        print;
    };
...
sub count {
    ...
    my $daoBackendHandle = $self->_getReadBackend()->{'handle'};
    try{
        $self->backendFactory->getInstance( $daoFactoryType, $daoBackendHandle )->getTeamDao()->count( $coderef );
    }
    catch{
        print;
    };
