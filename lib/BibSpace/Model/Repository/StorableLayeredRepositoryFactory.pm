...
package StorableLayeredRepositoryFactory;
...
sub hardReset {
    ...
    $self->idProviderAuthor->reset if defined $self->idProviderAuthor;
    $self->idProviderAuthorship->reset if defined $self->idProviderAuthorship;
    $self->idProviderEntry->reset if defined $self->idProviderEntry;
    $self->idProviderException->reset if defined $self->idProviderException;
    $self->idProviderLabeling->reset if defined $self->idProviderLabeling;
    $self->idProviderMembership->reset if defined $self->idProviderMembership;
    $self->idProviderTag->reset if defined $self->idProviderTag;
    $self->idProviderTagType->reset if defined $self->idProviderTagType;
    $self->idProviderTeam->reset if defined $self->idProviderTeam;
    $self->idProviderType->reset if defined $self->idProviderType;
...
sub getAuthorsRepository {
    ...
    if ( !defined $self->{_idProviderAuthor} ) {
        ...
        my $idProviderTypeClass
            = $backendsConfigHash->{'idProviderType'};
        try {
    ...
    if ( !defined $self->{_instanceAuthorsRepo} ) {
        ...
        $self->{_instanceAuthorsRepo} = AuthorsLayeredRepository->new(
            ...
            logger             => $self->logger,
            backendsConfigHash => $backendsConfigHash
        );
    }
