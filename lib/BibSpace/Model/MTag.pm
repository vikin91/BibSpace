...
sub update {
  ...
  if(!defined $self->{id}){
      say "Cannot update. MTag id not set. The entry may not exist in the DB. Returning -1";
      return -1;
  }
...
sub save {
  ...
  if(!defined $self->{id} or $self->{id} <= 0){
    ...
    warn "MTag save: cannot either insert nor update :( ID = ".$self->{id};
  }
}
####################################################################################
sub delete {
...
sub static_get_all_w_letter{
  ...
  my @params;
  my $qry = "SELECT id, name, type, permalink FROM Tag WHERE name IS NOT NULL AND type = ? ";
  push @params, $type;
  if(defined $letter){
      push @params, $letter;
