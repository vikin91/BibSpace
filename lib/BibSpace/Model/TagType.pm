...
sub static_get_by_name {
  ...
  return MTagType->new( id => $row->{id}, name => $row->{name}, comment => $row->{comment}, );
}
####################################################################################
sub static_get {
  my $self = shift;
  ...
  return MTagType->new( id => $id, name => $row->{name}, comment => $row->{comment}, );
}
####################################################################################
sub static_all {
  my $self = shift;
  ...
  while ( my $row = $sth->fetchrow_hashref() ) {
    push @objs, MTagType->new( id => $row->{id}, name => $row->{name}, comment => $row->{comment}, );
  }
  return @objs;
}
####################################################################################
sub update {
  ...
  if ( !defined $self->{id} ) {
    say "Cannot update MTagType: id not set. The entry may not exist in the DB. Returning undef";
    return undef;
  }
