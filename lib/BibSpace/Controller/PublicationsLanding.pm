...
sub num_pubs_filtering {
  ...
  return scalar Fget_publications_main_hashed_args( $self,
    { bibtex_type => $curr_bibtex_type, entry_type => $curr_entry_type, year => $curr_year, visible => 1, hidden => 0 }
  );
}
############################################################################################################
############################################################################################################
...
# my $entry_type = $self->param('entry_type') || 'paper';
  ...
  foreach my $yr (@allkeys) {
    ...
    my @objs = Fget_publications_main_hashed_args( $self,
      { year => $yr, entry_type => $entry_type, visible => 0, hidden => 0 } );
...
sub display_landing {
  ...
  $title .= " labeled as " . $display_tag_name           if defined $display_tag_name;
