  ...
  foreach my $section_name (sort reverse @section_names) {
    ...
    if ($section_name eq 'talk') {
      ...
      @entries_in_section = grep {
              $_->is_paper
          and $_->matches_our_type($section_name, $self->app->repo)
      } @entries_to_show;
    }
...
sub landing_years {
  ...
  my $min_year = $self->get_year_of_oldest_entry($author)
    // $self->current_year;
  ...
  my %hash_year_to_description = map { $_ => $_ } ($min_year .. $max_year);
...
sub get_switchlink_html {
  ...
  if ($keyword eq 'years') {
    $str
      .= $selected_anchor_delimiter_l
      . '<a class="landing_selected" href="'
      . $self->url_with('lp') . '">'
      . $selected_text_delimiter_l . 'Types'
      . $selected_text_delimiter_r . '</a>'
      . $selected_anchor_delimiter_r;
    $str
      .= $anchor_delimiter_l
      . '<a class="landing_normal"  href="'
      . $self->url_with('lyp') . '">'
      . $text_delimiter_l . 'Years'
      . $text_delimiter_r . '</a>'
      . $anchor_delimiter_r;
  }
    ...
    $str
      ...
      . '<a class="landing_selected" href="'
      . $self->url_with('lyp') . '">'
      . $selected_text_delimiter_l . 'Years'
      . $selected_text_delimiter_r . '</a>'
      . $selected_anchor_delimiter_r;
  }
  $str .= '</div>';
...
sub get_filtering_navbar_html {
  ...
  $str .= '<div>';
  my $url = $self->url_with->query(
    [bibtex_type => undef, entry_type => undef, year => undef]);
  $str
    .= $anchor_delimiter_l
    . '<a href="'
    . $url . '">'
...
sub get_navbar_kinds_html {
  ...
  foreach my $key (qw(Paper Talk)) {
    ...
    my $url;
    if ($key eq 'Talk') {
      $url = $self->url_with->query(
        [entry_type => lc($key), bibtex_type => 'misc']);
    }
    else {
...
sub get_navbar_types_html {
  ...
  foreach my $type (sort { $a->our_type cmp $b->our_type } @landingTypes) {
    ...
    my $text
      = $text_delimiter_l . $bibtex_type_to_label{$key} . $text_delimiter_r;

    if (defined $curr_bibtex_type and $key eq $curr_bibtex_type) {
      my $text
        = $selected_text_delimiter_l
        . $bibtex_type_to_label{$key}
        . $selected_text_delimiter_r;
      if ($num) {
        $str
          .= $selected_anchor_delimiter_l
          . '<a class="landing_selected" href="'
        ...
        $str
          .= $selected_anchor_delimiter_l
          . '<a class="landing_selected landing_no_papers" href="'
          . $url . '">'
          . $text . '</a>'
          . $selected_anchor_delimiter_r;
      }
      ...
      if ($num) {
        ...
        $str
          .= $anchor_delimiter_l
          . '<a class="landing_no_papers" href="'
          . $url . '">'
          . $text . '</a>'
          . $anchor_delimiter_r;
      }
...
sub get_navbar_years_html {
  ...
  foreach my $key (reverse sort @all_years) {
    my $num
      = $self->num_pubs_filtering($curr_bibtex_type, $curr_entry_type, $key);
