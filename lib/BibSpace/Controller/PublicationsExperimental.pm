...
sub publications_add_many_get {
  ...
  my $bib1 = '@article{key-'.random_string(8).'-'.get_current_year().',
  ...
  my $bib2 = '@article{key-'.random_string(8).'-'.get_current_year().',
      author = {Johny Example},
      title = {{Selected aspects of some methods '.random_string(8).'}},
      year = {'.get_current_year().'},
      month = {'.$mons{get_current_month()}.'},
...
sub publications_add_many_post {
  ...
  my $num_errors = 0;
  for my $bibtex_code (@bibtex_codes){
    my ($mentry, $status_code_str, $existing_id, $added_under_id) = Fhandle_add_edit_publication($dbh, $bibtex_code, -1, 'preview');

    if($status_code_str eq 'ERR_BIBTEX'){
      $debug_str.="<br>BIBTEX error in <br/><pre> $bibtex_code </pre><br/>";
      $num_errors = $num_errors+1;
    }
    elsif($status_code_str eq 'KEY_TAKEN'){ # => bibtex OK, key OK
      $debug_str.="<br>KEY_TAKEN error in <br/><pre> $bibtex_code </pre><br/>";
      $num_errors = $num_errors+1;
    }
  }

  if($num_errors > 0){
    $msg = $debug_str."Please correct entries before continuing. No changes were written to database.";
  ...
  for my $bibtex_code (@bibtex_codes){
    my ($mentry, $status_code_str, $existing_id, $added_under_id) = Fhandle_add_edit_publication($dbh, $bibtex_code, -1, 'save');

    if($status_code_str eq 'ADD_OK'){
      $debug_str.="<br>"."Added key entry as id $added_under_id successfully!";
    }
    else{ # => bibtex OK, key OK
      $debug_str.="<br>"."Something went wrong. Status: $status_code_str<br/>";
    }
  }
