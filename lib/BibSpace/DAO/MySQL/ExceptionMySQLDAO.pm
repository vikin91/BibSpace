...
sub count {
  my ($self) = @_;
  my $dbh    = $self->handle;
  my $sth    = $dbh->prepare("SELECT COUNT(*) as num FROM Exceptions_Entry_to_Team LIMIT 1");
  $sth->execute();
...
sub empty {
  my ($self) = @_;
  my $dbh = $self->handle;
  my $sth = $dbh->prepare("SELECT 1 as num FROM Exceptions_Entry_to_Team LIMIT 1");
  $sth->execute();
  my $row = $sth->fetchrow_hashref();
  my $num = $row->{num} // 0;
  return $num == 0;
...
sub exists {
  my ($self, $object) = @_;
  my $dbh = $self->handle;
  my $sth = $dbh->prepare("SELECT EXISTS(SELECT 1 FROM Exceptions_Entry_to_Team WHERE team_id=? AND entry_id=? LIMIT 1) as num ");
...
sub update {
  ...
  warn "".(caller(0))[3]." it makes no sense to update Exception! Method was instructed to update ".scalar(@objects)." objects.";
...
sub delete {
  ...
  my $dbh = $self->handle;
  foreach my $obj (@objects) {
    my $qry = "DELETE FROM Exceptions_Entry_to_Team WHERE entry_id=? AND team_id=?;";
    my $sth = $dbh->prepare($qry);
    try {
